...
- [Jakarta Persistence (JPA) Integration](#JPA)

  - [Project Setup](#JPA-Setup)

    - [Common Maven Coordinates](#JPA-Maven-Coordinates)
...
- [`@Named("test")`](https://jakarta.ee/specifications/dependency-injection/2.0/apidocs/jakarta/inject/named) says to use the data source named `test` (as declared by the [*datasourcename* portion](#DS-Configuration-Prefixes) of a named data source configuration property).
...
Jakarta Persistence requires [Jakarta Transactions](#JTA) and makes use of [named data sources](#DS), so as you set up your project you will need to understand:
...
2.  [Set up and configure Helidon MP’s Jakarta Transactions support](#JTA)
...
- [Ensure you have set up and configured named data sources as appropriate](#DS)

- [Ensure you have set up and configured Helidon MP’s Jakarta Transactions support](#JTA)
...
- [Ensure the basics of your JPA project are set up properly](#JPA-Maven-Coordinates)
...
Fundamentally, a `META-INF/persistence.xml` file contains a collection of *persistence units*. A persistence unit represents a collection of entities in a relational database loosely coupled to a [named data source](#DS) that knows how to connect to it.
...
A persistence unit is always associated with exactly one [named data source](#DS).

Because Helidon MP’s Jakarta Persistence integration provides support for container-managed JPA, and because container-managed JPA requires Jakarta Transactions (JTA), the kind of named data source a persistence unit is associated with is always a [JTA](#JTA) data source. The `<jta-data-source>` element, a child of the `<persistence-unit>` element, is how you link a persistence unit to a [named data source](#DS) you previously [configured](#DS-Configuration).
...
- This links this persistence unit to a [data source](#DS) named `main`, whose [connectivity information](#DS-Configuration) can be found in a MicroProfile-Config-compatible location, as detailed in the [data source configuration](#DS-Configuration) section above.
...
- The name identifies a name present in the [*datasourcename* portion of a named datasource configuration](#DS-Configuration-Prefixes). There is no need for any kind of reserved prefix (like `java:comp/env`).

- This is a Hibernate ORM-specific property and will be properly ignored if the JPA provider you have [set up](#JPA-Setup) is Eclipselink. See [Statement logging and statistics](https://docs.jboss.org/hibernate/orm/6.1/userguide/html_single/Hibernate_User_Guide.html#configurations-logging) in the Hibernate ORM documentation for more details about the `hibernate.show_sql` property.

- This is an Eclipselink-specific property (and (a) is required and (b) must be set to `false` if you are using Eclipselink), and will be properly ignored if the JPA provider you have [set up](#JPA-Setup) is Hibernate ORM. See [weaving](https://www.eclipse.org/eclipselink/documentation/4.0.2/jpa/extensions/persistenceproperties_ref.htm#weaving) in the Eclipselink documentation for more details about the `eclipselink.weaving` property.
...
To use Helidon MP’s Jakarta Persistence integration, once you have [set up](#JPA-Setup) and [configured](#JPA-Configuration) your project, you use the Jakarta Persistence APIs in almost the same manner as if your project were deployed to a Jakarta EE application server.
...
In addition, you [use Helidon MP’s JTA integration](#JTA-Usage) to declare transactional boundaries where appropriate.
